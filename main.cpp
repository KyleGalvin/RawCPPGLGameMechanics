...
QString OpenGL_Model("Skeleton.obj");
...
QString OpenGLES_Model("Skeleton.obj");
...
int main(int argc, char *argv[])
{
  ...
  Viewport *modelWindow = 0;

  qDebug() << "OpenGL Version:" << glVersion << (isOpenGLES ? "ES" : "Desktop");

  if (glVersion == qMakePair(3, 3) && !isOpenGLES) {
    modelWindow = new Viewport_GL_3_3(OpenGL_Model, ModelLoader::RelativePath);//new Scene(OpenGL_Model, ModelLoader::RelativePath);
  }
  ...
  // and we use a smaller model for OpenGL ES
    ...
    modelWindow = new Viewport_GL_2(OpenGLES_Model, ModelLoader::RelativePath);
  }
  else {
    modelWindow = new Viewport_GL_2(OpenGL_Model, ModelLoader::RelativePath);
  }

  modelWindow->initializeWindow(context, 40);
  modelWindow->showFullScreen();
  //modelWindow->show();
  a.connect(modelWindow, SIGNAL(quitApplication()), &a, SLOT(quit()));
